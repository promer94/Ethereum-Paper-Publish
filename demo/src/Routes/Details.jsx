...
class Details extends Component {
  ...
  renderBadge = (newAuthorRequest, isAgreed, contractAddress) => {
    if (newAuthorRequest && !isAgreed) {
      return (
        <Link
          className="waves-effect waves-light red-text text-darken-2 collection-item"
          to={`/ApproveNewAuthor/${contractAddress}`}
        >
          Approve New Author
        </Link>
      )
    } else if (!newAuthorRequest && !isAgreed) {
      return (
        <Link
          className="waves-effect waves-light red-text text-darken-2 collection-item"
          to={`/RequestNewAuthor/${contractAddress}`}
        >
          Add New Author
        </Link>
      )
  ...
  render() {
    ...
    const [address, versionCount] = this.props.address //eslint-disable-line
      .split(',')
      .map(v => v.trim())
    const { versionList, user, isPending, paperList } = this.props
    const { newAuthorRequest, isAgreed } =
      paperList && paperList.filter(v => v.address === address)[0]
    const contractAddress =
      paperList && paperList.filter(v => v.address === address)[0].address
    return (
        ...
        <div className="row">
          <div className="col s3 xl3 hide-on-small-only">
            ...
            <ul className="collection">
              ...
              {this.renderBadge(newAuthorRequest, isAgreed, contractAddress)}
            </ul>
          </div>
